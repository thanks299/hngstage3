    ...
    static async githubCallback(req, res) {
        ...
        try {
            ...
            if (user.rows.length === 0) {
                ...
                `, [githubUser.id.toString(), githubUser.login, primaryEmail?.email || null, githubUser.avatar_url, 'analyst', true]);
                ...
                await pool.query(`
                    ...
                    WHERE id = $4
                `, [githubUser.login, primaryEmail?.email || null, githubUser.avatar_url, user.id]);
            }
            ...
            const refreshExpiresAt = new Date(Date.now() + Number.parseInt(process.env.JWT_REFRESH_EXPIRY) * 1000);
            ...
            res.redirect(`${process.env.WEB_PORTAL_URL || 'http://localhost:3001'}/dashboard`);
        } catch (error) {
