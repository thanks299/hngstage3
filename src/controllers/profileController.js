...
class ProfileController {
    static async getAllProfiles(req, res) {
        try {
            let { page = 1, limit = 10, sort_by = 'created_at', order = 'DESC',
                    gender, country_id, age_group, min_age, max_age } = req.query;
            ...
            if (gender) {
                countQuery += ` AND gender = $${countIndex++}`;
                countParams.push(gender);
            }
            if (country_id) {
                countQuery += ` AND country_id = $${countIndex++}`;
                countParams.push(country_id.toUpperCase());
            }
            if (age_group) {
                countQuery += ` AND age_group = $${countIndex++}`;
    ...
    static async createProfile(req, res) {
        try {
            ...
            const values = [
                ...
                countryData.country_id ? await getCountryName(countryData.country_id) : null,
...
async function getCountryName(countryCode) {
    const countries = {
        ...
        'ZA': 'South Africa', 'AO': 'Angola', 'BJ': 'Benin',
        'CM': 'Cameroon', 'ET': 'Ethiopia', 'US': 'United States',
